...
// -------------------------------------------------------
// FORM LOCK — disables all inputs + add-row button
// -------------------------------------------------------
...
// BUTTON VISIBILITY
// State values:
//   "new"    → brand-new unsaved record  : Save, Cancel
//   "saved"  → saved draft               : Save, Edit, Post, Cancel
//   "posted" → posted record             : Void, Cancel
//   "void"   → voided record             : Cancel only
// -------------------------------------------------------
...
window.resetSalesForm = function (withInitialRow = true) {
  ...
  // Reset all confirm-states and button labels
  [
...
document.getElementById("saveSaleBtn").onclick = function () {
  ...
  window.DB.saveSale(sale, window.currentSaleIndex).then((id) => {
    // After first save, set index so subsequent edits update the same record
...
window.renderSalesList = function () {
  ...
  const filtered = window.savedSales.filter(s => {
    ...
    if (search &&
        ...
        !(s.reference || "").toLowerCase().includes(search)) return false;
    return true;
  });

  if (filtered.length === 0) {
    tbody.innerHTML = `<tr class="no-hover"><td colspan="4" class="empty-state">No sales found.</td></tr>`;
    return;
  }
...
document.getElementById("editSaleBtn").onclick = () => {
  lockSalesForm(false);
  toggleSalesButtons("saved");
};
